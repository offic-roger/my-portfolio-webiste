...
function getCategory(filename: string) {
  ...
  return m ? m[1] : "AI VIDEO";
}

const AI_BASE = "/portfolio/ai-videos";
const ANIM_3D_BASE = "/work/3d-animation";

const VIDEOS = [
  { file: "[STORY] THE POWER OF LIGHT SHELL .mp4", base: ANIM_3D_BASE },
  { file: "ai video happy new year.mp4",                base: AI_BASE },
  { file: "[MUSIC VIDEO] Ametenda - Atupele fred.mp4",  base: AI_BASE },
  { file: "[TANGAZO] Duka la vifaa vya magari.mp4",      base: AI_BASE },
  { file: "[TANGAZO] uhai drinking water.mp4",           base: AI_BASE },
].map(({ file, base }) => ({
  file,
  title: getTitle(file),
...
export default function ThreeDAndAIPage() {
  useLenis();
              ...
              >
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4 z-[3]">
                    ...
                    <p className="text-white font-semibold text-[14px] leading-tight">{video.title}</p>
                  </div>
                </div>
              </div>
            ))}
